...
function calculateStringFor2Items(data1, data2, type) {
    ...
    switch (type) {
        ...
        case "guns":
            var ammos = "", ammos2 = "";
            var ammos2_data = JSON.parse(JSON.stringify(data2.stats.weaponry.ammunition));
            console.log(ammos2_data);

            data1.stats.weaponry.ammunition.forEach(element => {
                ammo_stats = "";
                
                var foundElement = ammos2_data.find(function(Felement) {
                ...
                ammo_stats = "";
                for (var key of Object.keys(foundElement.stats)) {
                    if (foundElement.stats[key] == -1) continue;
                    
                    if (element.stats[key] | element.stats[key] != -1) {
                    ...
                    ammo_stats += `<tr><th>${ammo_stats_titles[key][0]}</th><td>${foundElement.stats[key]+ammo_stats_titles[key][1]}</td></tr>`;
                }
                
                ammos2 += `<tr><th colspan="3" class="stat_header">${foundElement.type}</th></tr>
                ...
                for (let index = 0; index < ammos2_data.length; index++) {
                    if (ammos2_data[index].type == foundElement.type) {
                        ammos2_data.splice(index, 1);
            ...
            console.log(data2.stats.weaponry.ammunition);

            ammos2_data.forEach(element => {
                ammo_stats = "";
